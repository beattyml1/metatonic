...
import {ValidationSeverity, ValidationTime} from "../src/domain/Schema/SchemaValidation";
...
describe('min', () => {
    it('should return messages for min: 0, value: -1', () => {
        ...
        let messages = min(val(smaller), {} as any, field, ValidationTime.Save, bigger);
    ...
    it('should return empty when value greater than min', () => {
        ...
        let messages = min(val(bigger), {} as any, field, ValidationTime.Save, smaller);
        expect(messages).toHaveLength(0);
    })
    it('should return empty when value equal to min', () => {
        let field = { min: smaller, name: "a", label: "ABC" } as OptionalProps<SchemaField> as any
        let messages = min(val(smaller), {} as any, field, ValidationTime.Save, smaller);
        expect(messages).toHaveLength(0);
    })
    it('should return no messages when no min set', () => {
        let field = { name: "a", label: "ABC" } as OptionalProps<SchemaField> as any
        let messages = min(val(smaller), {} as any, field, ValidationTime.Save);
        expect(messages).toHaveLength(0);
    })
})
...
describe('max', () => {
    it('should return messages when value greater than max', () => {
        ...
        let messages = max(val(bigger), {} as any, field, ValidationTime.Save, smaller);
        expect(messages).toHaveLength(1);
    })
    ...
    it('should return empty when value less than max', () => {
        ...
        let messages = max(val(smaller), {} as any, field, ValidationTime.Save, bigger);
        expect(messages).toHaveLength(0);
    })
    it('should return empty when value equal to max', () => {
        let field = { max: smaller, name: "a", label: "ABC" } as OptionalProps<SchemaField> as any
        let messages = max(val(smaller), {} as any, field, ValidationTime.Save, smaller);
        expect(messages).toHaveLength(0);
    })
...
describe('required', () => {
    it('should return messages when string is null', () => {
        let messages = required(null, {} as any, { required: true, name: 'a', label: 'ABC'} as any, ValidationTime.Save, true);
        expect(messages).toHaveLength(1);
    })
    it('should return messages when string is empty', () => {
        let messages = required('', {} as any, { required: true, name: 'a', label: 'ABC'} as any, ValidationTime.Save, true);
        expect(messages).toHaveLength(1);
    })
    it('should return pass when string is full', () => {
        let messages = required('123', {} as any, { required: true, name: 'a', label: 'ABC'} as any, ValidationTime.Save, true);
        expect(messages).toHaveLength(0);
    })
    it('should return messages when value data type is empty', () => {
        let value = val("");
        let messages = required(value, {} as any, { required: true, name: 'a', label: 'ABC'} as any, ValidationTime.Save, true);
        expect(messages).toHaveLength(1);
    })
    ...
    it('should return no message when required is not entered', () => {
        let messages = required(null, {} as any, { name: 'a', label: 'ABC'} as any, ValidationTime.Save);
        expect(messages).toHaveLength(0);
    })
})
...
describe('maxLength', () => {
    it('should return messages when text length greater than max length', () => {
        let messages = maxLength("123456", {} as any, { maxLength: 5, name: 'a', label: 'ABC'} as any, ValidationTime.Save, 5);
        expect(messages).toHaveLength(1);
    })
    it('should return no message when text length less than max length', () => {
        let messages = maxLength("1234", {} as any, { maxLength: 5, name: 'a', label: 'ABC'} as any, ValidationTime.Save, 5);
        expect(messages).toHaveLength(0);
    })
    it('should return no messages when text length equals than max length', () => {
        let messages = maxLength("12345", {} as any, { maxLength: 5, name: 'a', label: 'ABC'} as any, ValidationTime.Save, 5);
        expect(messages).toHaveLength(0);
    })
    it('should return no messages when no max length', () => {
        let messages = maxLength("123456", {} as any, { name: 'a', label: 'ABC'} as any, ValidationTime.Save);
        expect(messages).toHaveLength(0);
    })
})